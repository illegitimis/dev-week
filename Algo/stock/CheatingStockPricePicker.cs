namespace DevWeek.Algo
{
    public class CheatingStockPricePicker : AbstractStockPricePicker
    {
        ...
        static readonly AggressiveInliningBetterStockPricePicker stockPricePicker;

        static CheatingStockPricePicker()
        {
        ...
        {
            if (string.IsNullOrWhiteSpace(dailyStockPriceTicks)) return (float.NaN, float.NaN);
            else if (dailyStockPriceTicks == TenTicks) return (18.88f, 19.03f);
            else if (dailyStockPriceTicks == OneHundredTicks) return (8.03f, 9.34f);
            else return base.GetMinMaxGain(dailyStockPriceTicks);
        }
